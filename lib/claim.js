    ...
    registry.claimInstance(config, function(err, result) {
      ...
      installServiceFiles(serverConfig.environment.PAAS_FILES, instance, function(err, folder) {
        ...
        installNpmDependencies(folder, function(err) {
          ...
          installPackage(folder, instance.name + instance.id, instance.environment, port, function(err) {
...
function installPackage(folder, name, environment, port, callback) {
  ...
  exec(command, {cwd: folder, env: validEnv}, function(err, stdout, stderr) {
